...
```text
代码输入
  -> LLMTestGenerationAgent: 没有用户测试时，可选生成 pytest 测试脚本
  -> TestAgent: 运行测试、语法/合规检查、可选 AI 代码审查
  -> ReportAgent: 汇总测试/合规/AI 审查结果并生成结构化报告
  -> FixAgent: 根据报告生成修复并应用到临时沙箱
  -> VerifyAgent: 对修复后的沙箱重新执行确定性测试和合规检查
```

当前 LangGraph 流程：

```text
START -> LLMTestGenerationAgent -> TestAgent -> ReportAgent -> FixAgent -> VerifyAgent
VerifyAgent passed -> END
VerifyAgent failed and round < max_fix_rounds -> ReportAgent -> FixAgent -> VerifyAgent
```

## 已实现功能

- `LLMTestGenerationAgent`
  - 没有用户测试且启用 LLM 自测时，调用大模型生成 pytest 测试脚本。
  - 只负责生成和校验测试脚本，不运行测试。
  - 输出 `generated_test_result`，标记 `oracle_source=llm_inferred`、置信度和生成告警。

- `TestAgent`
  - 将输入代码写入临时沙箱目录。
  - 自动运行 Python 测试，优先使用 `pytest`，否则回退到 `unittest`。
  - 如果上游生成了 LLM 测试脚本，则写入沙箱并真实执行。
...
- `VerifyAgent`
  - 在 `FixAgent` 修复后的 `workspace_dir` 上重新执行测试和 `py_compile`。
  - 默认不重复调用 AI review，避免验证阶段成本过高。
  - 验证失败且未超过 `max_fix_rounds` 时，回到 `ReportAgent` 重新报告和修复。
...
.
  ...
  src/tcr_agent/
    ...
    agents/llm_test_generation_agent.py LLMTestGenerationAgent
    agents/test_agent.py              TestAgent
    agents/report_agent.py            ReportAgent
    agents/ai_code_review.py          AI 代码审查检查项
    agents/llm_test_generation.py     LLM 测试生成工具函数
...
```text
LLMTestGenerationAgent 推断行为并生成 pytest 测试脚本
TestAgent 写入 workspace/.tcr_generated_tests/test_generated_llm.py
TestAgent 运行 python -m pytest -q .tcr_generated_tests
...
generated_test_result
  LLMTestGenerationAgent 输出，记录生成阶段是否 attempted/generated、置信度、推断行为和测试脚本内容

generated_test_result.test_files
  生成的测试脚本，例如 path=test_generated_llm.py、content=<pytest 源码>

test_result
  TestAgent 输出，记录测试/合规/AI 审查的真实执行结果
