...
- Agent 使用 LangGraph 编排多个节点，形成可追踪的处理链路。
- 系统输出结构化报告、Issue 列表、修复 patch 和修复后文件。

整体链路：

```txt
上传代码文件 -> 创建任务 -> Agent DAG 执行 -> 生成报告 -> 生成修复结果 -> 前端展示
```
...
项目中的 Graph 指的是 LangGraph 编排的 Agent 执行图。当前实现是一个有向无环图，也就是 DAG。

DAG 的含义：

- 有向：每个节点之间有明确执行方向。
- 无环：执行链路不会回到之前节点，避免无限循环。
